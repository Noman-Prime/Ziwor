                    ...
                    <div className="relative flex flex-col items-center gap-6 lg:flex-row lg:items-center lg:justify-between">
                        <div className="flex flex-col items-center text-center lg:flex-row lg:items-center lg:gap-5 lg:text-start">
                            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-white/15 backdrop-blur-md sm:h-16 sm:w-16 lg:h-20 lg:w-20">
                                <MessageCircleMore
                                    size={30}
                                    className="text-white lg:h-10 lg:w-10"
                                />
                            </div>

                            <div className="mt-4 lg:mt-0">
                                <span className="inline-flex rounded-full bg-white/15 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.14em] text-white sm:text-xs">
                                    {t("badge")}
                                </span>

                                <h2 className="mt-3 text-2xl font-black leading-tight text-white sm:text-3xl lg:text-5xl">
                                    {t("title")}
                                </h2>

                                <p className="mt-2 max-w-2xl text-sm leading-6 text-white/90 sm:text-base lg:text-lg lg:leading-7">
                                    {t("description")}
                                </p>

                                {(businessName || supportHours) && (
                                    <p className="mt-3 text-xs font-semibold text-white/80 sm:text-sm">
                                        {businessName}
