        ...
        <nav className="relative z-50 w-full border-b-2 border-[#7A1E3A] bg-white">
            <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-4 sm:px-6 lg:h-[72px] lg:px-8 xl:h-20">
                ...
                <div className="flex items-center gap-1 sm:gap-2 lg:gap-3">
                    ...
                    <button type="button" aria-label={t("search")} className="flex h-10 w-10 items-center justify-center rounded-full text-[23px] text-[#222] transition hover:bg-[#F8EDF1] hover:text-[#7A1E3A] sm:text-[25px]">
                        <IoSearch />
                    </button>
                    <button type="button" onClick={() => navigateTo("/cart")} aria-label={t("cart")} className="relative flex h-10 w-10 items-center justify-center rounded-full text-[23px] text-[#222] transition hover:bg-[#F8EDF1] hover:text-[#7A1E3A] sm:text-[25px]">
                        <FiShoppingCart />
                        {!cartLoading && totalQuantity > 0 && <span className="absolute right-0 top-0 flex h-[18px] min-w-[18px] items-center justify-center rounded-full bg-[#C79A2B] px-1 text-[10px] font-bold leading-none text-white">{totalQuantity > 99 ? "99+" : totalQuantity}</span>}
                    </button>
            ...
            <div className={`absolute left-0 top-full w-full overflow-hidden bg-white shadow-xl transition-all duration-300 lg:hidden ${menuOpen ? "max-h-[850px] border-b border-[#EADCE2] opacity-100" : "pointer-events-none max-h-0 opacity-0"}`}>
                <div className="flex flex-col gap-3 px-4 py-5 sm:px-6">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-3 border-t border-[#EADCE2] pt-5">
                        <button type="button" onClick={() => navigateTo("/login")} className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#731D46] px-3 py-3 text-sm font-bold text-white transition hover:bg-[#D4A037]">
                            <FiLogIn size={16} />
                            <span>{t("login")}</span>
                        </button>
                        <button type="button" onClick={() => navigateTo("/signup")} className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#731D46] bg-white px-3 py-3 text-sm font-bold text-[#731D46] transition hover:bg-[#F8EDF1]">
                            <FiUserPlus size={16} />
