...
const Hero = ({ slides = [] }) => {
    ...
    const activeBadge =
        activeSlide?.badgeText || activeSlide?.badge || t("badge");
    const activeTitle = activeSlide?.title || t("titleLine1");
    const activeSubtitle =
        activeSlide?.subtitle || t("description");
    const activeButtonText =
        activeSlide?.buttonText || t("shopNow");
    const activeVariant =
        activeSlide?.product?.variant || null;

    const buyNowDisabled =
        cartActionLoading ||
        !activeSlide?.product?.availableForSale ||
        !activeVariant?.id ||
